...
- [x] **Analytics service ready:** container analytics service trả về `200` cho endpoint `/health` và `/stats` hoạt động.
...
- [x] **Network & Ports:** mạng `team-internal` hoạt động; API gọi được analytics bằng hostname `analytics-service`; ports 8000 (API), 9000 (Analytics) và 5432 (DB) được map đúng.
